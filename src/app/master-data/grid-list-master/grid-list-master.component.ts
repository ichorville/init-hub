...
@Component({
  selector: 'app-grid-list-master',
  templateUrl: './grid-list-master.component.html',
  styleUrls: ['./grid-list-master.component.css']
})
export class GridListMasterComponent implements OnInit {
  columns: any[];
	rows: any[];
	title: string;
  ...
  constructor(
		private _ggms: GridListMasterService,
  ...
  ngOnInit() {
    this.updateRows(this._ggms.gridList);
  }
  ...
  onLoad(event) {
	...
	add(event) {
		this.router.navigateByUrl(`/customizé/grids`);
	}

	update(event) {
		this.router.navigateByUrl(`/customizé/grids/${ event.name }`);
  }
