		...
		if (!term.trim()) {
			...
			this.terms.filter((element: object) => {
				...
				Object.getOwnPropertyNames(element).filter((match) => {
					if (this.searchSpecifics == undefined || this.searchSpecifics == null) {
						if (element[match] != undefined && element[match] != null && 
								element[match].toString().toLowerCase().indexOf(term.toLowerCase()) > -1) {
							// push to array if search condition passes
							arr.push(element);
						}	
					} else {
						if (element[match] != undefined && element[match] != null && match == this.searchSpecifics &&
								element[match].toString().toLowerCase().indexOf(term.toLowerCase()) > -1) {
							// push to array if search condition passes
							arr.push(element);
						}	
